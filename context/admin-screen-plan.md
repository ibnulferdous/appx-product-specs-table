...
- Onboarding checklist — four steps rendered as a `VerticalStack` of `Card` rows, in first-run execution order:
  1. ✅ / ⬜ **Create your first template** → opens the style gallery at `/app/templates/choose-style` <br>⚠️ **Corrected 2026-08-03 (step 107 D7).** This line read `/app/templates/new` until then, which predates feature 88. The gallery is **unskippable** by merchant decision and every Create affordance points at it; a checklist step that linked past it would reopen the skip path and stamp its templates `basedOnPreset: null`, which now means "the merchant chose Blank".
...
| Assign the template to a product | 🔴 **BLOCKED on OQ-107-B — do not build this signal yet.** This row reads "at least one `ProductAssignment` exists for the shop (equivalently, one `ProductAssignmentIndex` row with `status = APPLIED`)". The parenthetical is no longer an equivalence: **OQ-103-D** records that `ProductAssignmentIndex` has **zero references in application code** since the 2026-07-07 shop-level routing redesign, and proposes dropping the table. The likely resolution is that the signal keys off `ProductAssignment` alone — but that is a decision about this spec, not a rename. See `progress-tracker.md` §Open Questions. |
