...
- **Storefront = theme-inherit by default, merchant-overridable.** Storefront colors inherit the merchant's active theme with zero configuration, and the **Style tab** (`TableStyling`) lets the merchant override each surface (header / label / value / zebra-stripe backgrounds, border, label / value text). Overrides are emitted as the same kind of CSS variables on the table's scope wrapper, so a saved color and a default color flow through one mechanism. **Layout knobs are not colors** (row layout, section headers, collapsible, dividers, density — Style-tab spec 2026-07-18): they render as **modifier classes** on the same wrapper; keep the class set centralized in the shared storefront stylesheet, mirroring the CSS-variable discipline.
...
- **The editing grid never renders merchant styling.** The Edit view is a fixed editing surface; `TableStyling` is applied **only** in the Desktop / Tablet / Mobile previews and on the storefront. Do not wire a third consumer of `tableStylingCss.ts`. _(Added 2026-07-20 after feature 57 Step 11 was built and reverted — `context/features/67-…`. This is a separate rule from the no-hardcoded-hex line above, and deliberately so: the withdrawn implementation painted the grid using the shared `--appx-spec-*` variables, so it satisfied that rule completely. Colour centralization says where values come from; this says which surfaces they reach.)_
...
- **One narrow jsdom carve-out: DOM *glue*, never components.** Framework-free modules that only build, read and index a DOM tree (today `app/utils/valueDom.ts`) are faithfully testable in jsdom and are tested there. Such a file opts in for itself with `// @vitest-environment jsdom` on line 1 — the runner default stays `node`, so there is no second project and no second run. What jsdom can **not** model stays out and stays browser-verified: Polaris web components, and contenteditable *editing behavior* (the browser injecting a placeholder `<br>` into an emptied host, caret normalization, the native undo stack). Encode that browser behavior as a **fixture** — build the host the way the browser leaves it and assert our reading of it. A placeholder-`<br>` fixture is what turns the 2026-08-04 value-cell bug into a guarded regression rather than a story.
