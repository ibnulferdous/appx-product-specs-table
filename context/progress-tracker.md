...
- 🔴 **Stacked mode strips `<table>` semantics on the storefront — CARRIED OUT OF B1, UNRESOLVED (raised 2026-07-20, Step 12).** `rowLayout=STACKED` (and the mobile stacked layout) applies `display: block`, which removes the implicit table semantics, so a screen reader stops announcing row/column relationships. **This is the app's OWN markup**, not a merchant choice — unlike colour contrast (which Step 12 decided to leave alone entirely, and correctly), CLAUDE.md priority #2 puts this squarely on the app. It was deferred to Step 12 in writing and Step 12 did **not** resolve it: the rail a11y pass was admin-side, and fixing this means touching `spec-table.css` + the Liquid markup, which is storefront work needing its own live verification on real themes. **Do not let it dissolve into "B1 was signed off."** Likely shape of the fix: explicit ARIA roles (`role="table"`/`row`/`cell`) preserved across the `display: block` switch, verified with a screen reader. **SPEC WRITTEN 2026-07-20: `context/features/70-storefront-table-semantics-in-stacked-layouts.md`** — static unconditional ARIA role chain on the existing (already-correct) semantic markup, in **both** markup sites, plus a guard test that fails if a `display: block` class ever loses its role. Two things the spec pins that are easy to get wrong: the mobile rule is a **media query**, so a fix conditional on `rowLayout` would miss every narrow-screen shopper; and the spec carries an explicit **falsifier** — if explicit ARIA degrades the two-column control case in a real screen reader, the approach is wrong and reverts rather than gets patched. **Independent of B2** (ships no knob).
  - **CODE SHIPPED 2026-07-20** (`f6ac4aa`). Full role chain in **both** hand-mirrored markup sites; new `specTableAriaContract.test.ts` **parses `spec-table.css` for `display: block` rules** and requires every class it finds to carry a role in the Liquid — so a future stacked variant fails the suite rather than silently reintroducing the bug. Mutation-verified (dropping `role="cell"` fails, naming the class). 834 tests / 36 files, gate green, Theme Check unchanged (its 9 warnings are pre-existing, from the lazy-table pattern).
  - **Verified live on the storefront** (DJI Mavic 4 Pro — a `layout-stacked` + `collapsible` template, i.e. the exact broken case): roles present in the served DOM, and `display: block` confirmed active at desktop width. **Zero visual change proven by construction**, not by eye — neither stylesheet contains a `[role]` or `[aria-]` selector, so no rule can react to the new attributes.
  - 🔴 **STILL OPEN — the screen-reader pass has NOT been run.** Done-when #4 of `70-…` is unmet. Everything above verifies the attributes are *present and inert*; **none of it verifies that assistive tech actually announces the pairs**, which is the entire point. The spec's **falsifier is also unchecked**: explicit ARIA can *suppress* native table affordances in some readers, so the two-column control case must be compared before/after. **If it regresses, revert — do not patch** (the `<dl>` option goes back on the table). Needs NVDA or VoiceOver at desktop **and** ≤749px. **This question closes only when that runs.**
...
- **View toggle:** Edit is the only editable segment; Desktop/Mobile are **read-only storefront previews** (Phase D). No separate preview panel (WYSIWYG). **Tablet removed 2026-07-22** — the toggle is now Edit / Desktop / Mobile only (`deviceView.ts` `ViewId`). **Shared preview device (2026-07-22):** the chosen device (Desktop/Mobile) is one value shared across all three tabs, while edit-vs-preview is per-tab (`tabViewMemory.ts` `ViewMemory` = `{ device, modes }`) — Content still opens on the grid, Style/Settings still auto-open a preview, and picking a device on any tab moves every *previewing* tab to it; dropping a tab back to Edit affects only that tab and retains the shared device. **Browser-verified live 2026-07-22** (ACEFAST YF4 template on appx-dev): toggle shows only Edit/Desktop/Mobile; Style auto-opened Desktop; Mobile chosen on Style carried to Settings; Content stayed on the Edit grid; Mobile chosen on Content previewed there; and Content→Edit left Style/Settings on the retained Mobile.
