...
1. **Product assignment engine** (design locked 2026-07-07 — `data-model.md` §5/§9): **rigid, block-on-conflict, merchant-controlled** (Moon-Bundles style). One scope per template (all / selected products / type / vendor / selected collections); a dry-run **blocks activation** on any overlap with another ACTIVE template (O(rules) set-algebra + `products(query,first:1)` cross-dimension checks); DRAFT may hold a conflict, ACTIVE may not; **no merchant `priority` knob**. Delivery: broad rules → **one shop-level `[shop.metafields.app.routing]` json map** (O(1) writes, future products auto-covered), resolved in Liquid by handle via `metaobjects["$app:appx_spec_table"][handle]`; per-product `metaobject_reference` metafield only for bounded single-product overrides (batched `metafieldsSet` ≤25/call, or rate-limit-exempt `bulkOperationRunMutation` for >~2.5k). Rides Reshell Phase E. First build steps: `add-assignment` + `add-routing` migrations, `[shop.metafields.app.routing]` TOML def, then assignment UI + dry-run resolver.
