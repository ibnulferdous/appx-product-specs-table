...
- [ ] **Mandatory privacy webhooks registered and return 200** — `customers/data_request`, `customers/redact`, `shop/redact`, subscribed in `shopify.app.toml` under `compliance_topics` and handled by `app/routes/webhooks.customers.data_request.tsx`, `…customers.redact.tsx`, `…shop.redact.tsx` (steps 105–106). 🔴 **Only TWO of the three are no-ops.** The app stores no customer PII, so the two `customers/*` topics are genuine acknowledgements that touch no database — but **`shop/redact` does real work**: `eraseShopData` deletes the `Shop` row and cascades to templates, styling, assignments and routing, plus an explicit session sweep (`Session` has no FK). ⚠️ It is guarded on `isInstalled: false`, so a 200 does not imply data was deleted — a reinstalled shop is deliberately spared (merchant decision D1, `data-model.md` §"Data retention & erasure").
- [ ] **Invalid HMAC returns 401, not 200** — `authenticate.webhook` throws before any handler code runs, so this is free and the only way to lose it is a `try/catch` around that call. Guarded behaviourally by `app/routes/complianceWebhookRoutes.test.ts`; re-probe with a bad-signature `curl` before submission.
