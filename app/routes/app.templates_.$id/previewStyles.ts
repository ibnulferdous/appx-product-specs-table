...
/* Every container fallback is an OFF state, so an untouched table renders
   exactly as it did before these knobs existed: no width cap, no outer border,
   square corners.

   max-width, not width. A width of 900px overflows a 360px phone; a max-width
   of 900px caps a wide screen and shrinks below it, so the knob cannot collide
   with the 749px mobile breakpoint or with the stacked layouts.

   The outer border's color falls back THROUGH the row-rule color before
   reaching a literal: a merchant who sets only the shared Border swatch gets an
   outline that matches their row rules for free, and the dedicated swatch is
   there for the case where the frame should differ from the rules. */
.appx-spec-table {
  box-sizing: border-box;
  margin-block: 1rem;
  max-width: var(--appx-spec-table-max-width, 100%);
  border: var(--appx-spec-outer-border-width, 0) solid
    var(
      --appx-spec-outer-border-color,
      var(--appx-spec-border-color, rgba(0, 0, 0, 0.1))
    );
  border-radius: var(--appx-spec-outer-radius, 0);
}

/* --- Table alignment -------------------------------------------------------
   Only observable once a max-width is set, which is why the rail hides the
   control at full width. Auto inline margins, so the alignment is done by the
   box itself and needs nothing from the theme's container. All three members
   get a rule (defaults included) at equal specificity, per the file's design
   rules — LEFT is not left to the base rule's absence. */
...
.appx-spec-table--align-right {
  margin-inline: auto 0;
}

/* --- Outer border refinements ---------------------------------------------
   Presence flags, emitted only when the matching knob is set (see
   tableStylingCss.ts) — so neither rule can touch a table that has not opted
   in. */

/* The last visible row's own bottom rule sits directly on the wrapper border
...
/* A rounded corner does not clip a descendant's background on its own, so the
   section band and the stripe fills would square off the curve without this.
   Scoped to the flag: an over-wide table is better overflowing visibly than
   clipped where a shopper cannot reach it. */
...
/* Text only: no band, but the rule stays — this is the UNDERLINED look, and
   ...
   This is the ONLY member that paints the rule, so it is the only one the
   underline colour reaches (feature 96). The chain is nested rather than flat,
   and each link is load-bearing:

   - --appx-spec-header-underline-color is the dedicated knob, emitted only when
     a merchant sets it, so an untouched table never sees this link at all;
   - --appx-spec-border-color is the Divider color swatch, which dressed this
     rule outright before feature 96 — keeping it as the inner fallback is what
     makes the new knob a zero-repaint addition rather than a restyle;
   - currentColor is the pre-Step-3 literal and resolves to the title's own
     colour.

   That two-link chain is also why this swatch is the one control in the rail
   with no empty-state help text: no sentence short enough for the rail can
   state both links truthfully (see ColorKnob.emptyHelpText). */
...
/* The underlined member's summary counterpart, carrying the same nested colour
   chain as its flat twin above. Both shapes or neither: a merchant who set an
   underline colour and then enabled collapsing must not watch it revert to the
   Divider color, which is the same composition hazard feature 87 fixed for the
   band and the rule themselves. It is also what lets ONE visibility predicate
   cover both shapes in the rail. */
